    ...
    validateOrder: (id) =>
      Promise.resolve(
        id === ORDER
          ...
          : { ok: false as const, error: ClickError.OrderNotFound, note: "Order not found" },
      ),
    getByClickTransId: (id) => Promise.resolve(txs.get(id) ? { ...txs.get(id)! } : null),
    insertPrepared: (tx) => {
...
Deno.test("rejects a bad signature / wrong service id", async () => {
  ...
  const bad = await handleClickRequest({ ...p, sign_string: "deadbeef" }, CFG, md5, store);
...
Deno.test("prepare: success, unknown order, wrong amount, idempotent retry", async () => {
  const { store } = memoryStore();
  ...
  const unknown = await handleClickRequest(
    await signed({ click_trans_id: "ct-2", merchant_trans_id: "sub:ghost:brand_pro" }),
    CFG,
    md5,
    store,
  );
...
Deno.test("complete: fulfills once, duplicate → Already paid, effect not repeated", async () => {
...
Deno.test("complete: unknown transaction / prepare id mismatch → -6", async () => {
  const { store } = memoryStore();
...
Deno.test("complete with negative error from Click cancels the transaction", async () => {
...
Deno.test("unknown action → -3, missing params → -8", async () => {
  const { store } = memoryStore();
  const badAction = await handleClickRequest(await signed({ action: "7" }), CFG, md5, store);
