...
function memoryStore() {
  ...
  const store: PaymeStore = {
    now: () => ++clock,
    getTransaction: (id) => Promise.resolve(txs.get(id) ? { ...txs.get(id)! } : null),
    findPendingForAccount: (account, exclude) => {
      for (const tx of txs.values()) {
        if (tx.paycom_transaction_id === exclude || tx.state !== TxState.Created) continue;
        const same = account.deal_id
          ? tx.account.deal_id === account.deal_id
          : tx.account.user_id === account.user_id && tx.account.plan === account.plan;
    ...
    validateAccount: (account: PaymeAccount) => {
      if (account.user_id === "user-1" && account.plan === "brand_pro") {
        return Promise.resolve({ ok: true as const, expectedTiyin: PLAN_TIYIN });
      }
      return Promise.resolve({
        ok: false as const,
...
const rpc = (method: string, params: Record<string, unknown>, id = 1) => ({ id, method, params });

Deno.test("rejects unauthorized requests with -32504", async () => {
  const { store } = memoryStore();
  const res = await handlePaymeRpc(rpc("CheckPerformTransaction", { amount: 1, account }), store, false);
...
Deno.test("isAuthorized verifies Basic Paycom:<key>", () => {
  ...
  assertEquals(isAuthorized("Basic " + btoa("Hacker:secret"), ["secret"]), false);
  assertEquals(isAuthorized(null, ["secret"]), false);
  assertEquals(isAuthorized("Bearer abc", ["secret"]), false);
});

Deno.test("CheckPerformTransaction: allow / bad amount / bad account", async () => {
  const { store } = memoryStore();
  ...
  const badAccount = await handlePaymeRpc(
    rpc("CheckPerformTransaction", { amount: PLAN_TIYIN, account: { user_id: "nope", plan: "brand_pro" } }),
    store,
    true,
  );
...
Deno.test("CreateTransaction: creates, retries idempotently, blocks second pending tx", async () => {
  const { store } = memoryStore();
  const params = { id: "tx-1", time: 111, amount: PLAN_TIYIN, account };

  const first = await handlePaymeRpc(rpc("CreateTransaction", params), store, true);
  assertEquals(first.result?.state, TxState.Created);
  assertEquals(first.result?.transaction, "tx-1");

  const retry = await handlePaymeRpc(rpc("CreateTransaction", params), store, true);
...
Deno.test("PerformTransaction: performs once, repeat is idempotent, unknown → -31003", async () => {
  const { store, effects } = memoryStore();
  await handlePaymeRpc(rpc("CreateTransaction", { id: "tx-1", amount: PLAN_TIYIN, account }), store, true);

  const performed = await handlePaymeRpc(rpc("PerformTransaction", { id: "tx-1" }), store, true);
  assertEquals(performed.result?.state, TxState.Performed);
  assertEquals(effects.performed, 1);

  const repeat = await handlePaymeRpc(rpc("PerformTransaction", { id: "tx-1" }), store, true);
  ...
  const missing = await handlePaymeRpc(rpc("PerformTransaction", { id: "nope" }), store, true);
  assertEquals(missing.error?.code, PaymeError.TransactionNotFound);
});

Deno.test("CancelTransaction: before perform → -1, after perform → -2 with revert", async () => {
  const { store, effects } = memoryStore();

  await handlePaymeRpc(rpc("CreateTransaction", { id: "tx-1", amount: PLAN_TIYIN, account }), store, true);
  const cancelled = await handlePaymeRpc(rpc("CancelTransaction", { id: "tx-1", reason: 3 }), store, true);
  assertEquals(cancelled.result?.state, TxState.Cancelled);
  assertEquals(effects.reverted, 0);

  await handlePaymeRpc(rpc("CreateTransaction", { id: "tx-2", amount: PLAN_TIYIN, account }), store, true);
  await handlePaymeRpc(rpc("PerformTransaction", { id: "tx-2" }), store, true);
  const refunded = await handlePaymeRpc(rpc("CancelTransaction", { id: "tx-2", reason: 5 }), store, true);
  ...
  const repeat = await handlePaymeRpc(rpc("CancelTransaction", { id: "tx-2", reason: 5 }), store, true);
  assertEquals(repeat.result?.state, TxState.CancelledAfterPerform);
  assertEquals(effects.reverted, 1);

  // A cancelled transaction cannot be performed.
  const performCancelled = await handlePaymeRpc(rpc("PerformTransaction", { id: "tx-1" }), store, true);
...
Deno.test("CheckTransaction returns the full stored state", async () => {
  ...
  const res = await handlePaymeRpc(rpc("CheckTransaction", { id: "tx-1" }), store, true);
  ...
  const missing = await handlePaymeRpc(rpc("CheckTransaction", { id: "ghost" }), store, true);
  assertEquals(missing.error?.code, PaymeError.TransactionNotFound);
});
